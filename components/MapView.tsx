  ...
  if (Array.isArray(s.layers)) {
    s.layers = s.layers.filter((ly: any) => {
      ...
      if (ly?.type === "symbol") {
        ...
        const sl = String(ly?.["source-layer"] || "");
        if (KEEP_LABEL_RE.test(id) || KEEP_LABEL_RE.test(sl)) return true;
        // 有些底图（尤其是 MapTiler Streets v2）主要道路/区名标签的 layer id 不一定包含 “road/place”，
        // 但 filter 里会带道路等级（motorway/primary/secondary...）。兜底保留这些“主路/主区”标签。
        const filterText = ly?.filter ? JSON.stringify(ly.filter) : "";
        if (/(motorway|trunk|primary|secondary|tertiary|major|district)/i.test(filterText)) return true;
        if (dropRe.test(id) || dropRe.test(sl)) return false;
      }
      return true;
    });
    ...
    for (const ly of s.layers) {
      ...
      if (ly.type === "line") {
        ...
        if (/road|street|bridge|tunnel|motorway|trunk|primary|secondary/i.test(id)) {
          ly.paint["line-color"] = ly.paint["line-color"] ?? "#FFFFFF";
          ly.paint["line-gap-width"] = bumpWidth(ly.paint["line-gap-width"] ?? 0, 1 + (stroke / 100) * 0.4, 0.2);
        }
        if (/boundary|admin/i.test(id)) {
          ly.paint["line-color"] = "#111827";
          ly.paint["line-width"] = bumpWidth(ly.paint["line-width"], widthMul * 1.15, widthAdd + 0.8);
      ...
      if (ly.type === "symbol") {
        ...
        ly.paint["text-halo-width"] = 1.2 + (stroke / 100) * 1.8;
        ly.paint["text-color"] = ly.paint["text-color"] ?? "#111827";
        // 不强行覆盖 text-font：MapTiler 的 glyphs 里未必包含我们指定的字体栈，
        // 覆盖后可能导致 label 全部不渲染。中文显示交给 map.setLocalIdeographFontFamily。
      }
...
export default function MapView({ center, refreshKey, onSelect, lego: legoConfig }: Props) {
  const mapRef = useRef<MLMap | null>(null);
  const markersRef = useRef<Marker[]>([]);
  const popupRef = useRef<maplibregl.Popup | null>(null);
  const [items, setItems] = useState<Place[]>([]);
  const baseStyleRef = useRef<any | null>(null);
  const baseStyleUrlRef = useRef<string>("");
  ...
  const openPopup = useCallback(
    (p: Place) => {
      ...
      meta.style.marginBottom = "6px";
      const parts: string[] = [];
      if ((p as any).address) parts.push(String((p as any).address));
      if (typeof p.rating === "number") parts.push(`评分 ${p.rating}`);
      if (typeof p.price_per_person === "number") parts.push(`人均 ¥${p.price_per_person}`);
      if (p.tags?.length) parts.push(p.tags.slice(0, 6).join(" · "));
      meta.textContent = parts.join("  |  ");
  ...
  const lego = useMemo<LegoConfig>(() => {
    const envOn = String(process.env.NEXT_PUBLIC_MAP_LEGO_MODE || "").toLowerCase();
    const enabled = legoConfig?.enabled ?? ["1", "true", "yes", "on"].includes(envOn);
    const density = legoConfig?.density ?? Number(process.env.NEXT_PUBLIC_MAP_LEGO_DENSITY || 35);
    const stroke = legoConfig?.stroke ?? Number(process.env.NEXT_PUBLIC_MAP_LEGO_STROKE || 55);
    const saturation = legoConfig?.saturation ?? Number(process.env.NEXT_PUBLIC_MAP_LEGO_SATURATION || 70);
    return {
      enabled: Boolean(enabled),
      density: Number.isFinite(density) ? density : 35,
      stroke: Number.isFinite(stroke) ? stroke : 55,
      saturation: Number.isFinite(saturation) ? saturation : 70,
    };
  }, [legoConfig?.density, legoConfig?.enabled, legoConfig?.saturation, legoConfig?.stroke]);
  ...
  useEffect(() => {
    ...
    async function init() {
      let style: any = styleUrl || "https://demotiles.maplibre.org/style.json";

      // “乐高化”需要拿到 style.json 本体；拿不到就退化为原样式
      if (lego.enabled && typeof style === "string") {
        try {
          const resp = await fetch(style, { cache: "no-store" });
          const json = await resp.json();
          baseStyleRef.current = json;
          baseStyleUrlRef.current = style;
          style = legoifyStyle(json, lego);
        } catch {
          baseStyleRef.current = null;
          baseStyleUrlRef.current = "";
        }
      } else {
        baseStyleRef.current = null;
        baseStyleUrlRef.current = "";
      ...
      applyIdeographFontFamily(map as any);
      map.addControl(new maplibregl.NavigationControl(), "bottom-right");
      map.addControl(
        new maplibregl.GeolocateControl({ positionOptions: { enableHighAccuracy: true } }),
        "bottom-right"
  ...
  }, [center.lat, center.lng, lego.density, lego.enabled, lego.saturation, lego.stroke, styleUrl]);

  // 动态更新“乐高块面”参数：通过 setStyle 重新应用 style.json（marker/popup 不受影响）
  useEffect(() => {
    ...
    async function apply(map: MLMap) {
      const url = styleUrl || "https://demotiles.maplibre.org/style.json";
      if (!lego.enabled) {
        try {
          map.once("style.load", () => applyIdeographFontFamily(map));
          map.setStyle(url);
        } catch {}
        return;
      }

      // 如果 base style 不存在或 URL 变了，重新拉取
      if (!baseStyleRef.current || baseStyleUrlRef.current !== url) {
        try {
          const resp = await fetch(url, { cache: "no-store" });
          const json = await resp.json();
          baseStyleRef.current = json;
          baseStyleUrlRef.current = url;
        } catch {
          return;
        }
      }

      if (cancelled || !baseStyleRef.current) return;
      try {
        map.once("style.load", () => applyIdeographFontFamily(map));
        map.setStyle(legoifyStyle(baseStyleRef.current, lego));
      } catch {}
    }

    // 简单防抖，拖动 slider 时减少闪烁/请求
    t = setTimeout(() => void apply(m), 220);
